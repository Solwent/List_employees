...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(self,
                                 columns=('ID', 'name', 'email', 'number', 'salary'),
    ...
    def record(self, name, email, number, salary):
        self.db.insert_data(name, email, number, salary)
        self.view_records()

    # метод редактирования в БД
    def upd_record(self, name, email, number, salary):
        id = self.tree.set(self.tree.selection()[0], '#1')
        self.db.cur.execute('''
            UPDATE users SET name = ?, email = ?, number = ?, salary = ?
            WHERE id = ?
            ''', (name, email, number, salary, id))
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.entry_salary.place(x=240, y=140)
...
''')
    ...
    def insert_data(self, name, email, number, salary):
        self.cur.execute('''
                INSERT INTO users (name, email, number, salary)
                VALUES (?, ?, ?, ?)
''', (name, email, number, salary))
        self.conn.commit()
